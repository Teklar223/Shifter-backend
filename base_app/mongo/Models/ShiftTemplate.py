class Shift_Template:
    def __init__(self, team_id: int, template_name: str, start_hour: int, end_hour: int, needed_roles: list, color: str, template_id: str = None) -> None:
    ...
    def serialize(self):
        template_dict = {"TemplateID": self.__template_id, "TeamID": self.__team_id,
                "TemplateName": self.__template_name, "StartHour": self.__start_hour,
                  "EndHour": self.__end_hour, "NeededRoles": self.__needed_roles, "Color": self.__color}
    ...
    def deserialize(self, template_dict):
        ...
        team_id = int(template_dict["TeamID"])
        template_name = template_dict["TemplateName"]
        start_hour = int(template_dict["StartHour"])
        end_hour = int(template_dict["EndHour"])
        needed_roles = template_dict["NeededRoles"]
        color = template_dict["Color"]
        template = Shift_Template(team_id=team_id, template_name=template_name,
                                   start_hour=start_hour, end_hour=end_hour,
                                     needed_roles=needed_roles, color=color, template_id=template_id)
        return template